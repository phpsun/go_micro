...
static void MD5Init(MYMD5_CTX* mdContext)
{
...
static void MD5Update (MYMD5_CTX *mdContext, const unsigned char *inBuf, unsigned int inLen)
{
  uint32_t in[16];
  int mdi;
  unsigned int i, ii;
...
static void MD5Final (MYMD5_CTX *mdContext, unsigned char digest[16])
{
  uint32_t in[16];
  int mdi;
  unsigned int i, ii;
...
void MyMd5(const void* data, int datalen, unsigned char digest[16])
{
	MYMD5_CTX ctx;
	MD5Init(&ctx);
	MD5Update(&ctx, (const unsigned char*)data, (unsigned int)datalen);
	MD5Final(&ctx, digest);
}
